...
socket.emit("joinRoom", {
  ...
  room: sessionStorage.getItem("chatRoomId"),
});
...
function RandomChat() {
  ...
  const renderUsers = () => {
    ...
    // chatBoxRef.current.scrollTop = chatBoxRef.current.scrollHeight;
    // console.log(chatBoxRef.current.scrollTop);
    return roomUsers.map((user) => <li>{user.username}</li>);
  };

  const onLeave = () => {
    sessionStorage.setItem("chatRoomId", "");
  };
      ...
      <div className="chat-container">
        ...
        <main className="chat-main">
          <div className="chat-sidebar">
            ...
            <h2 id="room-name">{sessionStorage.getItem("chatRoomId")}</h2>
            {/* <h3>
